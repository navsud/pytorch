...
class TestSymNumberMagicMethods(TestCase):
    ...
    def test_dynamic_int_eager_usage(self):
        ...
        self.assertEqual(math.prod([2, 2], start=z), 8)
        ...
        self.assertEqual(y / 2.0, 0.5)
        self.assertEqual(float(z), 2.0)  # this & line above could return DynamicFloat
        self.assertFalse(not y)
        ...
        self.assertEqual(list(torch.nn.Linear(z, y)(torch.randn(z*2, z)).shape), [4, 1])
        self.assertEqual(list(torch.nn.Conv1d(4, 4, kernel_size=y, padding=x)(torch.randn(2, 4, 3)).shape), [2, 4, 3])
