...
AOTIDelegateExecutor::AOTIDelegateExecutor(
  ...
  if (device.str() == "cpu") {
    aoti_model_container_runner_ =
        std::make_unique<torch::inductor::AOTIModelContainerRunnerCpu>(
            model_path,
            executorConfig.maxNumConcurrentThreads,
            /* run_single_threaded= */ false);
  } else {
    TORCH_CHECK(
        create_aoti_model_container_runner_cuda != nullptr,
        "create_aoti_model_container_runner_cuda shouldn't be null if you are trying to run an aotinductor model with non-cpu device")
    aoti_model_container_runner_ = create_aoti_model_container_runner_cuda(
        model_path,
