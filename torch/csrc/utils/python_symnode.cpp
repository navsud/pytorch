...
py::handle get_dynint_class() {
  ...
  return storage
      .call_once_and_store_result([]() -> py::object {
        return py::module::import("torch.fx.experimental.sym_node").attr("DynamicInt");
      })
      .get_stored();
#else
  static py::handle symbool_class =
      py::object(py::module::import("torch.fx.experimental.sym_node").attr("DynamicInt")).release();
  return symbool_class;
#endif
}
